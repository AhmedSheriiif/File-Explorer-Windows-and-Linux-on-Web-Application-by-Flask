...
def filePage(var=""):
    ...
    if osWindows:
        ...
        for c in range(1, len(cList)):
            var_path += ' / <a style = "color:black;"href = "/files/' + '/'.join(cList[0:c + 1]) + '">' + unquote(
                cList[c]) + '</a>'
    ...
    return render_template('migration.html', currentDir=var, favList=favList, default_view_css_1=default_view_css_1,
                           default_view_css_2=default_view_css_2, view0_button=var1, view1_button=var2,
                           currentDir_path=var_path, dir_dict=dir_dict, file_dict=file_dict)


@app.route('/nav/<path:var>', methods=['GET'])
def navPage(var):
    global currentDirectory, osWindows
    ...
    return render_template('html/' + var + '.html')
